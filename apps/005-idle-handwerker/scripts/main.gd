...
const BG := Color("071522")
const SURFACE := Color("0d2236")
const SURFACE_2 := Color("14344f")
const TEXT := Color("f4f8fb")
const MUTED := Color("a7bac8")
const GREEN := Color("35b9e6") # Wasser / Kälte – primäre SHK-Akzentfarbe.
const GREEN_DARK := Color("102f49")
const GOLD := Color("f28c28") # Wärme / Heizung – sekundäre SHK-Akzentfarbe.
const RED := Color("e85d5d")
...
func _notification(what: int) -> void:
	...
	if what == NOTIFICATION_APPLICATION_PAUSED or what == NOTIFICATION_WM_CLOSE_REQUEST:
		game.save_game()
		if monetization:
			monetization.handle_application_paused(true)
	elif what == NOTIFICATION_APPLICATION_RESUMED and monetization:
		monetization.handle_application_paused(false)
...
func _build_header() -> Control:
	...
	brand_eyebrow = _label("KAMILUNAVO · SHK SIMULATION", 10, GREEN)
...
func _build_dashboard() -> void:
	...
	progress_bar.add_theme_stylebox_override("background", _box(Color("081827"), 6))
	...
	primary_button.add_theme_color_override("font_color", Color("ffffff"))
...
func _goal_card(item: Dictionary, daily: bool) -> Control:
	...
	bar.add_theme_stylebox_override("background", _box(Color("081827"), 4))
...
func _team_progress_card() -> Control:
	...
	progress.add_theme_stylebox_override("background", _box(Color("081827"), 5))
...
func _upgrade_card(upgrade: Dictionary) -> Control:
	...
	row.add_theme_constant_override("separation", 13)
	card.get_child(0).get_child(0).add_child(row)
	row.add_child(_icon_box(str(upgrade.icon), GREEN))
	var copy := VBoxContainer.new()
	copy.size_flags_horizontal = Control.SIZE_EXPAND_FILL
...
func _show_location_restart_confirmation(location_id: String) -> void:
	...
	if location.is_empty():
		return
	_show_action_modal(
		"NEUER STANDORT · %s" % str(location.code),
		"Betrieb in %s eröffnen?\n\nGeld, Ausbau, Personal, Verträge, laufende Aufträge und Serie starten bei null. Betriebsstufe, Ruf, Bewertungen, Erfolge und Meisterpunkte bleiben erhalten." % str(location.title),
		"BETRIEB ERÖFFNEN",
		func() -> void:
...
func _show_offline_dialog() -> void:
	...
	collect.add_theme_color_override("font_color", Color("ffffff"))
...
func _show_reset_confirmation(settings_overlay: Control) -> void:
	...
	_show_action_modal(
		...
		"Geld, Ruf, Aufträge, Team, Meisterpunkte und alle Verbesserungen werden dauerhaft gelöscht. Diese Aktion kann nicht rückgängig gemacht werden.",
		"DAUERHAFT LÖSCHEN",
		reset_game,
		true
	)
...
func _show_action_modal(title: String, body: String, confirm_text: String, confirmed: Callable, destructive := false) -> void:
	var overlay := ColorRect.new()
	overlay.color = Color("030b12e8")
	overlay.mouse_filter = Control.MOUSE_FILTER_STOP
	overlay.set_anchors_and_offsets_preset(Control.PRESET_FULL_RECT)
	...
	overlay.add_child(center)
	var panel := PanelContainer.new()
	panel.custom_minimum_size = Vector2(minf(400.0, size.x - 28.0), 330.0)
	panel.add_theme_stylebox_override("panel", UiSkin.hero())
	center.add_child(panel)
	var margin := MarginContainer.new()
	_set_margins(margin, 24, 24, 24, 22)
	panel.add_child(margin)
	var box := VBoxContainer.new()
	box.add_theme_constant_override("separation", 16)
	margin.add_child(box)
	var badge := _label("SHK · BETRIEBSENTSCHEIDUNG", 10, GREEN, true)
	badge.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	box.add_child(badge)
	var heading := _label(title, 22, TEXT, true)
	heading.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	heading.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	heading.text_overrun_behavior = TextServer.OVERRUN_NO_TRIMMING
	box.add_child(heading)
	var copy := _label(body, 12, TEXT)
	copy.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	copy.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	copy.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	copy.text_overrun_behavior = TextServer.OVERRUN_NO_TRIMMING
	copy.size_flags_vertical = Control.SIZE_EXPAND_FILL
	box.add_child(copy)
	var confirm_button := Button.new()
	confirm_button.text = confirm_text
	confirm_button.custom_minimum_size.y = 56
	_style_small_button(confirm_button)
	if destructive:
		confirm_button.add_theme_color_override("font_color", Color("ffffff"))
		confirm_button.add_theme_stylebox_override("normal", _box(RED, 16, Color("ff9999"), 1))
	...
	cancel_button.add_theme_color_override("font_color", TEXT)
	cancel_button.add_theme_stylebox_override("normal", UiSkin.dark_button())
	cancel_button.add_theme_stylebox_override("pressed", UiSkin.pressed_button())
...
func _show_tutorial() -> void:
	...
	next.add_theme_color_override("font_color", Color("ffffff"))
...
func _show_toast(message: String) -> void:
	...
	panel.add_theme_stylebox_override("panel", _box(Color("eaf6fc"), 14))
	panel.position = Vector2(24, size.y - 160)
	panel.size = Vector2(size.x - 48, 54)
	var label := _label(message, 12, Color("0b2437"), true)
...
func _milestone_card() -> Control:
	...
	bar.add_theme_stylebox_override("background", _box(Color("081827"), 5))
...
func _avatar(letter: String, color: Color) -> Control:
	...
	var label := _label(letter, 20, Color("071522"), true)
	label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	panel.add_child(label)
	return panel
