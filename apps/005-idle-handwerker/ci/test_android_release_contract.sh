...
grep -Fq "id: screenshots" "$workflow"
grep -Fq "continue-on-error: true" "$workflow"
grep -Fq "steps.screenshots.outcome == 'failure'" "$workflow"
grep -Fq 'screenshot_outcome: ${{ steps.screenshots.outcome }}' "$workflow"
grep -Fq "needs.build-and-screenshot.outputs.screenshot_outcome == 'success'" "$workflow"
