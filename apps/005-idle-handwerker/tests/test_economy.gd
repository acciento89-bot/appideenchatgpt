...
var android_events: Array[String] = []


class FakeAndroidPurchase extends RefCounted:
	var data: Dictionary
...
class FakeAndroidIap extends Node:
	var available_purchases: Array = []
	var purchase_result: Variant = null
	...
	func get_available_purchases() -> Array:
		return available_purchases

	func request_purchase(_props: Variant) -> Variant:
		return purchase_result

	func finish_transaction_dict(purchase: Dictionary, consumable: bool) -> Dictionary:
		events.append("finish:%s:%s" % [purchase.get("purchaseToken", ""), consumable])
		return {"success": true}
...
func _record_android_grant(_product_id: String, transaction_id: String) -> void:
...
func _init() -> void:
	...
	storekit.free()
	var android := MonetizationBridge.new()
	...
	if not android_events.is_empty() or android._pending_product_id.is_empty():
		failures += 1
		printerr("FAIL: pending Android purchase must remain pending and ungranted")
	...
	if android_events != ["grant:token-fallback", "finish:token-fallback:true"]:
		failures += 1
		printerr("FAIL: Android grant must use token fallback and persist before consume")
	android_events.clear()
	fake_iap.available_purchases = [FakeAndroidPurchase.new({
		"productId": small_product,
		"purchaseState": "purchased",
		"purchaseToken": "restored-consumable",
	})]
	await android._restore_android_purchases(false)
	...
	if not android._pending_product_id.is_empty():
		failures += 1
