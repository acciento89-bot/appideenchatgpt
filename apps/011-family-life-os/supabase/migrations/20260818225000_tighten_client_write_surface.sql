...
-- -----------------------------------------------------------------------------
-- Restrict authenticated clients to product-editable columns. Processing state,
-- provenance and review-finalization fields remain server/RPC-owned.
-- -----------------------------------------------------------------------------

revoke update on public.households from authenticated;
grant update (name, locale, timezone) on public.households to authenticated;

revoke update on public.source_items from authenticated;
grant update (display_title, original_text, storage_path) on public.source_items to authenticated;
...
-- Direct authenticated plan inserts may be manual items with no provenance, or
-- may carry provenance only when source + proposal belong to the same household.
...
with check (
    ...
    and (
        (source_item_id is null and source_proposal_id is null)
        or exists (
            select 1
            from public.action_proposals p
            join public.source_items s on s.id = p.source_item_id
            where p.id = source_proposal_id
              and s.id = source_item_id
              and s.household_id = plan_items.household_id
        )
    )
);

-- -----------------------------------------------------------------------------
-- Canonical confirmation owns review_status/source processing/provenance writes.
-- SECURITY DEFINER is required because those columns are intentionally not
-- writable by the authenticated client role. Explicit household checks below
-- remain mandatory because the function owner can bypass table RLS.
-- -----------------------------------------------------------------------------
