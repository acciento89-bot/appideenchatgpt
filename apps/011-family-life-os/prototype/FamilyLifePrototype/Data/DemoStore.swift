...
final class DemoStore {
    ...
    var entitlement: FamilyEntitlement
    ...
    var includedProposalCount: Int { proposals.filter { $0.isIncluded && $0.reviewStatus == .proposed }.count }
    var hasBlockingProposal: Bool {
        proposals.contains { $0.isIncluded && $0.reviewStatus == .proposed && !$0.isReadyToConfirm }
    }
    ...
    func member(for id: UUID) -> FamilyMember? { members.first { $0.id == id } }
    ...
    func openSignatureReview() {
        if let source = inboxItems.first(where: { $0.status == .review }) { openReview(sourceID: source.id) }
    }
        ...
        Task {
            do {
                let snapshot = try await repository.ingestText(
                    TextIngestionRequest(title: "Klassenfahrt 6b · Text", text: Self.signatureSourceText, createdByMemberID: actor.id)
                )
                apply(snapshot)
                if let newSource = inboxItems.filter({ !existingSourceIDs.contains($0.id) }).sorted(by: { $0.createdAt > $1.createdAt }).first {
    ...
    func confirmSelectedProposals() {
        guard !hasBlockingProposal, let selectedSourceID else { return }
        let reviewed = proposals
        let acceptedCount = reviewed.filter { $0.isIncluded && $0.reviewStatus == .proposed && $0.isReadyToConfirm }.count
        isRepositoryBusy = true
        repositoryErrorMessage = nil
        Task {
            do {
                let snapshot = try await repository.confirmReviewedProposals(sourceID: selectedSourceID, proposals: reviewed)
                apply(snapshot)
    ...
    func toggleCompletion(_ itemID: UUID) {
        ...
        Task {
            do { apply(try await repository.setPlanItemCompleted(itemID, isCompleted: desiredState)) }
            catch {
                if let rollbackIndex = planItems.firstIndex(where: { $0.id == itemID }) { planItems[rollbackIndex].isCompleted = previousState }
    ...
    func addChild(named name: String) {
        ...
        repositoryErrorMessage = nil
        Task {
            do { apply(try await repository.addChild(named: cleanName)) }
            catch { repositoryErrorMessage = error.localizedDescription }
            isRepositoryBusy = false
        }
    }

    func applySnapshot(_ snapshot: FamilySnapshot) { apply(snapshot) }
    ...
    private func apply(_ snapshot: FamilySnapshot) {
        ...
        notificationPreferences = snapshot.notificationPreferences
        allProposals = snapshot.proposals
        if let selectedSourceID { proposals = snapshot.proposals.filter { $0.sourceID == selectedSourceID } }
        else { proposals = snapshot.proposals }
    }
    ...
    private static func apply(_ scenario: DemoScenario, to snapshot: inout FamilySnapshot) {
        ...
        case .standard: break
        case .calmToday:
            guard let lina = snapshot.members.first(where: { $0.name == "Lina" }), let ben = snapshot.members.first(where: { $0.name == "Ben" }) else { return }
            snapshot.planItems = [
        ...
        case .conflictToday:
            guard let mara = snapshot.members.first(where: { $0.name == "Mara" }), let lina = snapshot.members.first(where: { $0.name == "Lina" }), let ben = snapshot.members.first(where: { $0.name == "Ben" }) else { return }
            snapshot.planItems = [
    ...
    private static func makeFixture() -> FamilySnapshot {
        ...
        let schoolLetterID = UUID(), screenshotID = UUID(), voiceID = UUID(), failedID = UUID(), partialID = UUID(), queuedID = UUID()
        let inbox = [
        ...
        return FamilySnapshot(
            household: HouseholdSummary(id: UUID(), name: "Familie Berger", locale: "de-DE", timezone: "Europe/Berlin"),
